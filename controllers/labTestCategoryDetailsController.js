...
exports.createLabTestCategoryDetails = async (req, res) => {
  ...
  try {
    ...
    if (!labTest) {
      ...
      logger.logWithMeta("error", "Invalid LabTestID, not found in MasterDB", {
        errorCode,
        executionTime,
        hospitalId: req.hospitalName,
        apiName: req.originalUrl,
    ...
    if (!labTestCategory) {
      ...
      logger.logWithMeta("error", "Invalid LabTestCategoryIDR, not found in MasterDB", {
        errorCode,
        executionTime,
        hospitalId: req.hospitalName,
        apiName: req.originalUrl,
      ...
      return res.status(400).json({
        errorCode,
        message: "Invalid LabTestCategoryIDR, not found in MasterDB",
      });
    }
    ...
    const result = await createLabTestCategoryDetailsDAO(req.sequelize, labTestCategoryDetailsData);
    ...
    res.status(201).json({
      message: "Lab Test Category Details created successfully",
      meta: {
        statusCode: 200,
        executionTime,
        hospitalDatabase,
    ...
    logger.logWithMeta("error", "Error creating Lab Test Category Details", {
      errorCode,
      executionTime,
      hospitalId: req.hospitalName,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      ...
      error: { message: "Error creating Lab Test Category Details  : " + error.message },
    });
  }
};
...
exports.getAllLabTestCategoryDetails = async (req, res) => {
  ...
  try {
    ...
    const result = await getAllLabTestCategoryDetailsDAO(req.sequelize);
    const executionTime = `${Date.now() - start}ms`;

    logger.logWithMeta("info", "Fetched Lab Category Details successfully", {
      executionTime,
      hospitalId: req.hospitalName,
      apiName: req.originalUrl,
      city: locationData?.city,
    ...
    res.status(200).json({
      message: "All Lab Category Details Fetched successfully",
      meta: {
        statusCode: 200,
        executionTime,
        hospitalDatabase,
    ...
    logger.logWithMeta("error", "Error fetching Lab Test Category Details", {
      errorCode,
      executionTime,
      hospitalId: req.hospitalName,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      meta: { statusCode: 500, errorCode, executionTime, hospitalDatabase },
      error: { message: "Error fetching Lab Test Category Details: " + error.message },
    });
  }
};
...
exports.getLabTestCategoryDetailsById = async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const result = await getLabTestCategoryDetailsByIdDAO(req.sequelize, id);

    if (!result) {
      const executionTime = `${Date.now() - start}ms`;
      const errorCode = 1262;

      logger.logWithMeta("error", "Lab Test Category Details not found", {
        errorCode,
        executionTime,
        hospitalId: req.hospitalName,
        apiName: req.originalUrl,
      ...
      return res.status(404).json({
        errorCode: 1263,
        message: "Lab Test Category Details not found in Database",
        hospitalDatabase,
      });
    }
    const executionTime = `${Date.now() - start}ms`;

    logger.logWithMeta("info", "Fetched Lab Test Category Details successfully", {
      executionTime,
      hospitalId: req.hospitalName,
      apiName: req.originalUrl,
      city: locationData?.city,
...
const labTest = await getLabTestByIdDAO(req.sequelize, req.body.labTestIDR);
    ...
    const updated = await updatLabTestCategoryDetailsByIdDAO(
      req.sequelize,
      id,
      labTestCategoryDetailsData
    );

    const executionTime = `${Date.now() - start}ms`;

    logger.logWithMeta("info", "Lab Test Category Details updated successfully", {
      hospitalDatabase,
      executionTime,
      apiName: req.originalUrl,
    });
    ...
    if (!updated) {
      ...
      logger.logWithMeta("error", "Invalid Lab Test Category Details id, not found in DB", {
        errorCode,
        executionTime,
        apiName: req.originalUrl,
        city: locationData?.city,
      ...
      return res.status(400).json({
        errorCode,
        message: "Invalid Lab Test Category Details id, not found in DB",
      });
    }

    res.status(200).json({
      message: "Lab Test Category Details Updated Successfully",
      meta: { statusCode: 200, executionTime, hospitalDatabase },
      data: labTestCategoryDetailsGET(updated),
    });
    ...
    logger.logWithMeta("error", "Error updating Lab Test Category Details", {
      errorCode,
      executionTime,
      hospitalDatabase,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      meta: { statusCode: 500, errorCode, executionTime, hospitalDatabase },
      error: { message: "Error updating Lab Test Category Details: " + error.message },
    });
  }
};
...
exports.deleteLabTestCategoryDetails = async (req, res) => {
  ...
  const { id } = req.params;
  try {
    const deleted = await deleteLabTestCategoryDetailsByIdDAO(req.sequelize, id);
    if (!deleted) {
      const executionTime = `${Date.now() - start}ms`;
      const errorCode = 9245;
      ...
      logger.logWithMeta(
        "error",
        "Invalid Lab Test Category Details id, not found in Database",
        {
          errorCode,
          executionTime,
          ID: req.params.id,
    ...
    logger.logWithMeta("info", "Lab Test Category Details Deleted successfully", {
      executionTime,
      ID: req.params.id,
      apiName: req.originalUrl,
      city: locationData?.city,
    ...
    res.status(200).json({
      meta: { statusCode: 200, executionTime, hospitalDatabase },
      message: "Lab Test Category Details deleted successfully",
    });
  } catch (error) {
    const executionTime = `${Date.now() - start}ms`;
    const errorCode = 9249;

    logger.logWithMeta("error", "Error Deleting Lab Test Category Details", {
      errorCode,
      executionTime,
      hospitalDatabase,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      meta: { statusCode: 500, errorCode, executionTime, hospitalDatabase },
      error: { message: "Error Deleting Lab Test Category Details: " + error.message },
    });
  }
};
...
exports.getLabTestCategoryDetailsByQueryParams = async (req, res) => {
  ...
  try {
    ...
    const fieldMap = labTestCategoryDetailsFieldMap;

    const ID_DTO_FIELD = "labTestCategoryDetailsId"; // Ensure this matches your DTO
    ...
    const { count: totalRecords, rows } =
      await getLabTestCategoryDetailsDataAsPerQueryParamDAO(req.sequelize, {
        attributes,
        ...pagination,
      });
    ...
    const responseData = rows.map((record) => {
      const fullDto = labTestCategoryDetailsGET(record);
      const filteredDto = {};
