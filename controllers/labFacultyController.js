...
exports.updateLabFacultyById = async (req, res) => {
  ...
  try {
    ...
    logger.logWithMeta("info", "Lab Faculty updated successfully", {
      hospitalDatabase,
      executionTime,
      apiName: req.originalUrl,
    });
    ...
    if (!updated) {
      ...
      logger.logWithMeta("error", "Invalid Lab Faculty id, not found in DB", {
        errorCode,
        executionTime,
        apiName: req.originalUrl,
        city: locationData?.city,
    ...
    res.status(200).json({
      message: "Lab Faculty Updated Successfully",
      meta: { statusCode: 200, executionTime, hospitalDatabase },
      data: labFacultyGET(updated),
    });
  } catch (error) {
    const executionTime = `${Date.now() - start}ms`;
    const errorCode = 9249;

    logger.logWithMeta("error", "Error updating Lab Faculty", {
      errorCode,
      executionTime,
      hospitalDatabase,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      meta: { statusCode: 500, errorCode, executionTime, hospitalDatabase },
      error: { message: "Error updating Lab Faculty: " + error.message },
    });
  }
};

exports.deleteLabFacultyById = async (req, res) => {
  const start = Date.now();
  const clientIp = await getClientIp(req);
  const locationData = await getLocationData(clientIp);
  const hospitalDatabase = req.hospitalDatabase;
  const { id } = req.params;
  try {
    const deleted = await deleteLabFacultyByIdDAO(req.sequelize, id);
    if (!deleted) {
      const executionTime = `${Date.now() - start}ms`;
      const errorCode = 9245;
      ...
      logger.logWithMeta(
        "error",
        "Invalid Lab Faculty id, not found in Database",
        {
          errorCode,
          executionTime,
    ...
    res.status(200).json({
      meta: { statusCode: 200, executionTime, hospitalDatabase },
      message: "Lab Faculty deleted successfully",
    });
  } catch (error) {
    const executionTime = `${Date.now() - start}ms`;
    const errorCode = 9249;

    logger.logWithMeta("error", "Error Deleting Lab Faculty", {
      errorCode,
      executionTime,
      hospitalDatabase,
      apiName: req.originalUrl,
    ...
    res.status(500).json({
      meta: { statusCode: 500, errorCode, executionTime, hospitalDatabase },
      error: { message: "Error Deleting Lab Faculty: " + error.message },
    });
  }
};

exports.getLabFacultyDataAsPerQueryParam = async (req, res) => {
  const start = Date.now();
  const clientIp = await getClientIp(req);
  const hospitalDatabase = req.hospitalDatabase;
  const locationData = await getLocationData(clientIp);
  try {
    ...
    const fieldMap = labFacultyFieldMap;

    const ID_DTO_FIELD = "labFacultyId"; // Ensure this matches your DTO
    ...
    const { count: totalRecords, rows } =
      await getLabFacultyDataAsPerQueryParamDAO(req.sequelize, {
        attributes,
        ...pagination,
      });
