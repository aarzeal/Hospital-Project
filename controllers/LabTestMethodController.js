...
exports.getAllLabTestMethod = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "All Lab test methods Fetched successfully",
      meta: {
        statusCode: 200,
        executionTime,
        hospitalDatabase,
  ...
  try {
    ...
    if (!result) {
      ...
      return res
        ...
        .json({
          errorCode: 1263,
          message: "Lab test method not found in Database",
          hospitalDatabase,
        });
    }
    const executionTime = `${Date.now() - start}ms`;
...
exports.updateLabTestMethodById = async (req, res) => {
  ...
  try {
    ...
    if (!updated) {
      ...
      logger.logWithMeta(
        "error",
        "Invalid lab test method id, not found in DB",
        {
          errorCode,
          executionTime,
          ID: req.params.lab_test_method_id,
...
exports.deleteLabTestMethodById = async (req, res) => {
  ...
  try {
    ...
    if (!deleted) {
      ...
      logger.logWithMeta(
        "error",
        "Invalid lab test method id, not found in Database",
        {
          errorCode,
          executionTime,
          ID: req.params.lab_test_method_id,
...
exports.getCustomDataAsPerQueryParam = async (req, res) => {
  ...
  try {
    ...
    const attributes = [
      ...new Set(requestedDtoFields.map((dtoField) => fieldMap[dtoField])),
    ];
    ...
    const { count: totalRecords, rows } =
      await LabTestMethodDao.getCustomDataAsPerQueryParamDAO(req.sequelize, {
        attributes,
        ...pagination,
      });
